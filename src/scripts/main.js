...
function loadGallery() {
    ...
    for (const p of o) {
        ...
        if (!data)
            continue;
        const slide = document.createElement("div"), name = document.createElement("div"), image = document.createElement("img"), mask = document.createElement("div"), imageWrapper = document.createElement("div");
        ...
        function handleSlideEvent(event) {
            if (event.button === 0) {
                ...
                slide.classList.toggle("highlight");
        }
    ...
    if (hashSlide)
        hashSlide.scrollIntoView({ block: "start" });
}
function updateViewer() {
    ...
    const name = document.querySelector("#viewer > .name-wrapper > .name"), title = document.querySelector("#viewer > .name-wrapper > .title"), image = document.querySelector("#viewer > .image-wrapper > .image"), typesDiv = document.querySelector("#viewer > .types"), abilitiesDiv = document.querySelector("#viewer > .abilities"), entries = document.querySelector("#viewer > .entries-panel > .entries-outer > .entries");
    name.textContent = data.name;
    ...
    for (const a of data.abilities) {
        const aData = abilities[a], ability = document.createElement("div");
        ability.textContent = aData.name;
        ability.addEventListener("pointerdown", (event) => {
            if (event.button !== 0 || searchBar.value.includes(aData.name))
                return;
    ...
    for (const t of data.types) {
        ...
        type.addEventListener("pointerdown", (event) => {
            if (event.button !== 0 || searchBar.value.includes(tData.name))
                return;
    ...
    const evolvesInto = data.evolvesInto || [];
    const evolvesFrom = data.evolvesFrom || [];
    const hashSlide = slidesMap[p];
...
function registerInput(input, callback, store = true, immediate = true) {
    if (store) {
        const storedValue = getStorage(`input-${input.id}`);
        if (storedValue !== null)
            setInput(input, storedValue);
...
function sortOrder(order, reverse = false) {
    return (a, b) => {
...
export const pokemon = await loadJson("src/data/pokemon.json"), types = await loadJson("src/data/types.json"), abilities = await loadJson("src/data/abilities.json"), orders = await loadJson("src/data/orders.json");
let hashP = location.hash.substring(1);
export let title = document.title;
window.addEventListener("resize", updateOrientation);
await DOMContentLoaded();
updateOrientation();
export const gallery = document.getElementById("gallery"), viewer = document.getElementById("viewer"), coolStuff = document.getElementById("cool-stuff-toggle"), searchBar = document.getElementById("search-bar"), orderInput = document.getElementById("order"), reverseInput = document.getElementById("reverse");
...
registerInput(coolStuff, (event) => {
    if (event.target.checked)
        html.setAttribute("cool-stuff", "");
    else
        html.removeAttribute("cool-stuff");
});
